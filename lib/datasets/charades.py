...
class Charades(data.Dataset):
    ...
    def __init__(self, split):
        ...
        with open(os.path.join(self.data_dir, 'charades_bert_tokens_{}.json'.format(split)), 'r') as f:
            annos = json.load(f)
        with open(os.path.join(self.data_dir, 'charades_bert_tokens.pkl'), 'rb') as f:
            self.pkl_file = pickle.load(f)

        annotations = []
        vid_keys = set()
        size = int(round(len(annos) * 1.))
        for row in annos[:size]:
            ...
            vid = row['video']
            vid_keys.add(vid)
            s_time = row['time_start']
            e_time = row['time_end']
            fps = row['fps']
            num_frames = row['number_frames']
            sent = row['description']
            sent = sent.split('.\n')[0]
            sent_idx = row['sent_idx']
            sent_vec = self.get_sentence_features(sent_idx)
            duration = num_frames / fps
            self.durations[vid] = duration
            # iou2d = moment_to_iou2d(moment, num_clips, duration)
            # words = embedding(sent)
            # sentence = self.get_sentence_features(sent_idx, pkl_file)
            # query = sentence
            annotations.append(
                {'video': vid, 'times': [s_time, e_time], 'description': sent, 'duration': duration, 'sent_vec': sent_vec})
    ...
    def __getitem__(self, index):
        ...
        sent_vec = self.annotations[index]['sent_vec']
    ...
    def get_video_features(self, vid):
        ...
        if self.vis_input_type == 'c3d':
            features = torch.from_numpy(hdf5_file[vid]['c3d_fc6_features'][:]).float()
        else:
    ...
    def get_sentence_features(self, sent_idx):
        feature = self.pkl_file[sent_idx]['sent_feature']
        return feature