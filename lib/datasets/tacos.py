...
class TACoS(data.Dataset):
    ...
    def __init__(self, split):
        ...
        with open(os.path.join(self.data_dir, 'tacos_new_bert_25_{}.pkl'.format(split)), 'rb') as f:
            self.pkl_file = pickle.load(f)
        anno_pairs = []
        for vid, video_anno in annotations.items():
            if vid not in self.pkl_file.keys():
                continue
            duration = video_anno['num_frames']/video_anno['fps']
            for timestamp, sent, sent_vec in zip(self.pkl_file[vid]['timestamps'], self.pkl_file[vid]['sentences'], self.pkl_file[vid]['vector']):
