from core.timer import Timer, get_time_str
...
class Engine(object):
    ...
    def test(self, network, iterator, split):
        ...
        for sample in state['iterator']:
            ...
            closure()
            state['t'] += 1
        inference_time = inference_timer.toc()
        inference_time_str = get_time_str(inference_time)
        print("inference_time = {}".format(inference_time_str))
